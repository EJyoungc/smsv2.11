...
class PaymentController extends Controller
{
    ...
    public function pay_now(Request $req, $pr_id)
    {
        ...
        $payment = $this->pay->find($pr[0]->payment_id);
        // dd($req->all(),$req->amt_paid,$pr_id, $pr[0]->payment_id, $pr, $payment);

        $d['amt_paid'] = $amt_p = $pr[0]->amt_paid + $req->amt_paid;
        $d['balance'] = $bal = $payment->amount - $amt_p;
        $d['paid'] = $bal < 1 ? 1 : 0;
        // dd(, $d);
        $this->pay->updateRecord($pr_id[0], $d);
